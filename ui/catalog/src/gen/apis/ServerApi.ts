...
export class ServerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async bootstrap(bootstrapRequest: BootstrapRequest, _options?: Configuration): Promise<RequestContext> {
        ...
        if (bootstrapRequest === null || bootstrapRequest === undefined) {
            throw new RequiredError("ServerApi", "bootstrap", "bootstrapRequest");
        }


        // Path Params
        const localVarPath = '/management/v1/bootstrap';
    ...
    public async getServerInfo(_options?: Configuration): Promise<RequestContext> {
        ...
        // Path Params
        const localVarPath = '/management/v1/info';
...
export class ServerApiResponseProcessor {
     ...
     */
     public async bootstrapWithHttpInfo(response: ResponseContext): Promise<HttpInfo<void >> {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getServerInfoWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Array<ServerInfo> >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Array<ServerInfo> = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "Array<ServerInfo>", ""
            ) as Array<ServerInfo>;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
