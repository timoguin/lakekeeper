"""Shared helpers for the agentic-memory example.
...
* Two **AI agents** authenticate non-interactively with the client-credentials
  grant (`get_token()`) — the right model for autonomous, no-human-in-the-loop
  work:
    - `AGENT_A` writes and reads its own memory; may read shared memory and
      approved skills; may propose a skill.
    - `AGENT_B` the same, in its own scope. It cannot see agent A's memory, and
      neither agent can approve a skill.

Peter is also the **reviewer**: the only principal with write access to
`skills.approved`. That single asymmetry is what makes promotion a human act.
"""
...
WAREHOUSE_NAME = os.environ.get("WAREHOUSE_NAME", "agentmem")
...
# Namespaces.
#
# The isolation boundary is the TABLE, so anything holding one scope's content —
# entries and embeddings alike — lives inside that scope's own namespace. A
# shared recall table would let agent B vector-search agent A's memories.
NS_MEMORY = "agent_memory"
NS_AGENT_A = f"{NS_MEMORY}.agent_a"
NS_AGENT_B = f"{NS_MEMORY}.agent_b"
NS_SHARED = f"{NS_MEMORY}.shared"
MEMORY_SCOPES = (NS_AGENT_A, NS_AGENT_B, NS_SHARED)

# Skills: one central library. Each agent writes only its own proposal queue; `approved`
# is read-only for them — peter alone can write it, which is the whole demo.
NS_SKILLS = "skills"

# The triage rules the governance agent enforces, kept as governed data rather than code.
# Deciding what counts as suspicious is the real privilege here, so it gets the same
# treatment as everything else: the governance agent reads it and cannot change it.
NS_POLICY = "governance"
POLICY_TABLE = "policy"
...
# keycloak/realm.json — demo only, never reuse. Neither agent has any special
# power; peter grants them exactly what they hold.
AGENT_A = ("agent-a", "agent-a-secret")
AGENT_B = ("agent-b", "agent-b-secret")
AGENTS = {"agent-a": AGENT_A, "agent-b": AGENT_B}

# The governance agent: reads every proposal queue and every memory scope, and can write
# nothing. It triages the review queue so a human reads the three that matter instead of
# forty — but approving stays a human act, enforced the same way everything else here is.
GOVERNANCE = ("governance", "governance-secret")
...
def namespace_id(token: str, name: str) -> str:
    ...
    Multi-level names ("agent_memory.agent_a") are joined with the Iceberg REST unit
    separator rather than a dot, which the server rejects.
    """
    wh = warehouse_id(token)
    encoded = name.replace(".", "\x1f")
    r = requests.get(
        f"{CATALOG_URL}/v1/{wh}/namespaces/{encoded}", headers=auth(token), timeout=15
    )
    r.raise_for_status()
