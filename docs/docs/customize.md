...
* **Admission gates** allow or deny an already-authenticated principal before any handler runs. This is separate from the per-resource `Authorizer`. Use one to ask an external control plane whether a caller may use this instance, to suspend a tenant or principal, or to reject revoked tokens. A gate returns a terminal `403`, or a `503` with a `Retry-After` when it fails closed because an upstream it needs is unreachable. If a gate governs only some principals — one identity provider, tenant or path — return `GateDecision::not_applicable()` for the rest instead of `GateDecision::admit()`. Both let the request through, but only the first keeps the requests the gate approved apart from the ones it never examined. Gates run on every authenticated request, so cache aggressively and set your own timeouts. Every gate evaluation is timed for you (`lakekeeper_admission_gate_duration_seconds`); a gate's own cache can report into Lakekeeper's shared `lakekeeper_cache_*` series via the helpers in `lakekeeper::service::cache_metrics`. See [Monitoring](./monitoring.md#admission-gate-metrics).
