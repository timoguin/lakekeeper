...
use iceberg_catalog::modules::catalog_backends::postgres::{get_reader_pool, MigrationState};
...
pub(crate) async fn wait_for_db(
    ...
    if check_migrations {
        ...
        loop {
            ...
            let migrations =
                iceberg_catalog::modules::catalog_backends::postgres::check_migration_status(
                    &read_pool,
                )
                .await;
            match migrations {
