...
use crate::{
    modules::{authz::Authorizer, secrets::SecretStore, CatalogBackend},
    WarehouseIdent,
};
...
pub struct CatalogServer<C: CatalogBackend, A: Authorizer + Clone, S: SecretStore> {
...
pub(crate) mod test {
    ...
    use crate::modules::catalog_backends::postgres::{
        ...
        ApiContext {
            v1_state: State {
                ...
                queues: TaskQueues::new(
                    Arc::new(
                        crate::modules::catalog_backends::postgres::task_queues::TabularExpirationQueue::from_config(ReadWrite::from_pools(pool.clone(), pool.clone()), CONFIG.queue_config.clone()).unwrap(),
                    ),
                    Arc::new(
                        crate::modules::catalog_backends::postgres::task_queues::TabularPurgeQueue::from_config(ReadWrite::from_pools(pool.clone(), pool), CONFIG.queue_config.clone()).unwrap()
                    )
                )
            },
        }
