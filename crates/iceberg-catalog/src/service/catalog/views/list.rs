...
use crate::service::catalog::namespace::validate_namespace_ident;
use crate::service::catalog::require_warehouse_id;
use futures::FutureExt;
use iceberg_ext::catalog::rest::{IcebergErrorResponse, ListTablesResponse};

pub(crate) async fn list_views<C: CatalogBackend, A: Authorizer + Clone, S: SecretStore>(
    ...
    authorizer
        ...
        .await?;
    let mut t: <C as CatalogBackend>::Transaction =
    ...
    let (identifiers, view_uuids, next_page_token) =
        crate::service::catalog::fetch_until_full_page::<_, _, _, _, C>(
            query.page_size,
            query.page_token,
            |ps, page_token, trx| {
